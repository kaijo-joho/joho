...
(() => {
  ...
  function getIconConf(key) {
    ...
    if (!pick) return { src: '', emoji: DEFAULT_EMOJI[key], svg: '', useDefault: true };
    ...
    return { src: '', emoji: DEFAULT_EMOJI[key], svg: '', useDefault: true };
  }

  function createIconNode(key) {
    const { src, emoji, svg, useDefault } = getIconConf(key);

    if (useDefault) {
      const icon = window.siteHeaderMenus?.createIcon?.(key);
      if (icon) return icon;
    }
  ...
  function sectionTitle(title, iconKey) {
    return el('h3', { class: 'ld-sec__title' }, [
      window.siteHeaderMenus?.createIcon?.(iconKey),
      el('span', { class: 'ld-sec__label' }, title)
    ]);
  }

  function secList(title, items, iconKey) {
    const sec = el('div', { class: 'ld-sec' });
    sec.appendChild(sectionTitle(title, iconKey));
  ...
  function secWorksheet(title, items) {
    const sec = el('div', { class: 'ld-sec' });
    sec.appendChild(sectionTitle(title, 'worksheet'));
  ...
  function secFaq(title, items, course, idSuffix = '') {
    ...
    sec.appendChild(sectionTitle(title, 'faq'));
  ...
  function secNexts(nextList) {
    const sec = el('div', { class: 'ld-sec' });
    sec.appendChild(sectionTitle('次回', 'next'));
