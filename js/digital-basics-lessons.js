(function () {
  ...
  function setupQuestionGroup(root) {
    ...
    const Radix = window.DigitalRadixCore, Integer = window.DigitalIntegerCore, Real = window.DigitalRealCore, Text = window.DigitalTextCore, Color = window.DigitalColorCore;
    ...
    if (forms.some(form => one(form, '[data-db-answer-part="color"]')) && !Color) return;
        ...
        if (parts.length) {
          const results = parts.map(part => {
            ...
            if (type === 'bytes') {
              ...
              value = Color.parseHex(part.value)?.hex ?? null;
              expectedValue = Color.parseHex(part.dataset.dbExpected)?.hex ?? null;
            } else if (type === 'text') {
            ...
            if (type !== 'bytes' && type !== 'text' && type !== 'color') expectedValue = bits ? part.dataset.dbExpected : Number(part.dataset.dbExpected);
