...
ヘッダーの検索アイコンは、ホバー・クリック・タップ・キーボードで検索メニューを開きます。「教材サイト内検索を開く」で検索モーダルへ進み、その下にはFAQの検索欄・一覧へのリンク・現在ページに関連する質問を表示します。FAQは教材メニューには重複表示しません。開閉とフォーカスの仕様は[ヘッダーの共通ナビゲーション](header-navigation.md)を参照してください。

FAQ部分は`js/lesson_dock.js`の既存のデータ解決とUIを利用します。FAQ検索は`faq.html?q=...&course=...`へ渡し、講座を特定できないページでは`course`を省略します。FAQ本文の検索処理をサイト内検索へ複製せず、`js/faq.js`・`js/faq_page.js`は入口の移動では変更しません。FAQbotやその`returnUrl`へ検索語を引き継がない仕様も維持します。

検索メニューを開いただけではサイト内検索の索引を取得しません。サイト内検索UIが利用できない場合も、FAQ検索と一覧へのリンクは利用できます。
