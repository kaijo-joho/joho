...
ok(!pages[0].includes('lc01') && !pages[0].includes('lc03') && !pages[0].includes('lc04'), 'cp21に旧LC教材の内部表記を残さない');
ok(!pages[2].includes('lc04'), 'cp22に旧LC04教材の内部表記を残さない');
for (const script of ['./js/logic-lessons.js', './js/logic-quiz.js']) {
  ok(pages[0].includes(`<script defer src="${script}"></script>`), `cp21が${script}を初期化`);
}
for (const id of ['logic-quiz-tabs', 'panel-single', 'panel-table', 'panel-build', 'cp21-quiz-guide-dialog']) {
  ok(pages[0].includes(`id="${id}"`), `cp21に${id}`);
}
...
ok(applications.includes("['truth', 'function', 'name', 'complete']"), 'cp22に4段階の表示状態');
ok(applications.includes('logic-application-card--${stage}'), 'cp22は現在の段階をカードのクラスへ反映');
for (const stage of ['function', 'name', 'complete']) {
  ok(applications.includes(`setCardStage('${stage}'`), `cp22が次の段階 ${stage} へ進む`);
}

for (const oldPage of ['lc00.html', 'lc01.html', 'lc03.html', 'lc04.html', 'cp31.html', 'cp32.html']) {
