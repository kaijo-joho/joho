...
(function (root, factory) {
  ...
  function validatePayload(value, allowLegacy = false) {
    ...
    if (value.schemaVersion !== 1 || !validTimestamp(value.savedAt) ||
        ...
        typeof value.content !== 'string' || byteLength(JSON.stringify(value)) > MAX_BYTES) {
  ...
  function legacyPayload(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw) || typeof raw.content !== 'string' ||
        ...
        byteLength(raw.content) > MAX_BYTES) throw new Error('旧下書きの値が不正です');
    return {
  ...
  function create(storage, options = {}) {
    ...
    function save(snapshot) {
      ...
      if (typeof snapshot.content !== 'string' || byteLength(snapshot.content) > MAX_BYTES) throw new Error('保存データが大きすぎます');
      const payload = { ...snapshot, schemaVersion: 1, savedAt };
      validatePayload(payload);
      if (byteLength(JSON.stringify(payload)) > MAX_BYTES) throw new Error('保存データが大きすぎます');
