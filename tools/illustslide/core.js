(function (root, factory) {
  ...
  function validateLayers(value, objects) {
    const layers = array(value, 'page.layers'); if (!layers.length || layers.length > 100) fail('page.layers has an invalid length');
    const known = new Set(objects.map(object => object.id)), layerIds = new Set(), objectIds = new Set();
    const out = layers.map((layer, index) => {
      const label = 'page.layers[' + index + ']'; plainObject(layer, label); keysOnly(layer, ['id','name','visible','locked','objectIds'], label);
      const checked = { id: id(layer.id, label + '.id'), name: string(layer.name, label + '.name', true), visible: bool(layer.visible, label + '.visible'), locked: bool(layer.locked, label + '.locked'), objectIds: array(layer.objectIds, label + '.objectIds').map((objectId, objectIndex) => id(objectId, label + '.objectIds[' + objectIndex + ']')) };
      if (layerIds.has(checked.id)) fail('duplicate layer id: ' + checked.id); layerIds.add(checked.id);
      checked.objectIds.forEach(objectId => { if (!known.has(objectId)) fail('unknown layer object id: ' + objectId); if (objectIds.has(objectId)) fail('duplicate layer object id: ' + objectId); objectIds.add(objectId); }); return checked;
    });
    if (objectIds.size !== known.size) fail('layer object membership is incomplete');
    const membership = new Map(); out.forEach(layer => layer.objectIds.forEach(objectId => membership.set(objectId, layer.id)));
    const groups = new Map(); objects.forEach(object => { if (object.group === null) return; const previous = groups.get(object.group), current = membership.get(object.id); if (previous !== undefined && previous !== current) fail('group crosses layers: ' + object.group); groups.set(object.group, current); });
    const canonical = out.flatMap(layer => layer.objectIds); if (canonical.some((objectId, index) => objectId !== objects[index].id)) fail('page.objects はレイヤー順と一致する必要があります');
    return out;
  }
  ...
  function transformObjects(page, ids, transform) { const selected = new Set(expandSelection(page, ids)); const m = matrix(transform, 'matrix'); page.objects.forEach(o => { const layer=layerForObject(page,o.id); if (selected.has(o.id) && !o.locked && (!layer || layer.visible&&!layer.locked)) transformObject(o,m); }); return Array.from(selected); }
  ...
  function layerForObject(page, objectId) { if (!Array.isArray(page.layers)) return null; return page.layers.find(layer => layer.objectIds.includes(objectId)) || null; }
  function reconcileLayers(page) { if (!Array.isArray(page.layers)) return page; const byId = new Map(page.objects.map(object => [object.id, object])), assigned = new Set(); page.layers.forEach(layer => { layer.objectIds = layer.objectIds.filter(objectId => byId.has(objectId) && !assigned.has(objectId) && (assigned.add(objectId), true)); }); const fallback = page.layers.find(layer => layer.visible && !layer.locked) || page.layers[0]; page.objects.forEach(object => { if (!assigned.has(object.id)) fallback.objectIds.push(object.id); }); const ordered = []; page.layers.forEach(layer => layer.objectIds.forEach(objectId => ordered.push(byId.get(objectId)))); page.objects.splice(0, page.objects.length, ...ordered); return page; }
  function ensureSameLayer(page, objectIds) { if (!Array.isArray(page.layers)) return; const layers = new Set(objectIds.map(objectId => layerForObject(page, objectId)?.id)); if (layers.size > 1) throw new RangeError('異なるレイヤーのオブジェクトはグループ化できません。'); }
  function assertLayerMutable(page, objectIds) { if (!Array.isArray(page.layers)) return; objectIds.forEach(objectId => { const layer=layerForObject(page,objectId); if (!layer || !layer.visible || layer.locked) throw new RangeError('非表示または固定されたレイヤーのオブジェクトは変更できません。'); }); }
  ...
  function duplicateObjects(page, ids, dx, dy) { const selected = new Set(expandSelection(page, ids)); assertLayerMutable(page,selected); const groups = new Map(), map=new Map([...selected].map(id=>[id,uid('object')])), sourceLayers=new Map(); if(Array.isArray(page.layers))selected.forEach(objectId=>sourceLayers.set(objectId,layerForObject(page,objectId))); const copies = []; const offset = [1, 0, 0, 1, dx === undefined ? 10 : finite(dx, 'dx'), dy === undefined ? 10 : finite(dy, 'dy')]; page.objects.forEach(o => { if (!selected.has(o.id)) return; const sourceId=o.id, copy = clone(o); copy.id = map.get(sourceId); if (copy.group !== null) { if (!groups.has(copy.group)) groups.set(copy.group, uid('group')); copy.group = groups.get(copy.group); } transformObject(copy,offset);remapConnections(copy,map);copies.push(copy);if(sourceLayers.has(sourceId))sourceLayers.get(sourceId).objectIds.push(copy.id); }); page.objects.push(...copies);duplicateAnimations(page,map);reconcileLayers(page); return copies.map(o => o.id); }
  function groupObjects(page, ids) { const selected = expandSelection(page, ids); if (selected.length < 2) return selected; assertLayerMutable(page,selected); ensureSameLayer(page, selected); const group = uid('group'); const set = new Set(selected); page.objects.forEach(o => { if (set.has(o.id)) o.group = group; }); return selected; }
  function ungroupObjects(page, ids) { const selected = new Set(expandSelection(page, ids)); assertLayerMutable(page,selected); page.objects.forEach(o => { if (selected.has(o.id)) o.group = null; }); return Array.from(selected); }
  function removeObjects(page, ids) { const selected = new Set(expandSelection(page, ids)); assertLayerMutable(page,selected); page.objects.splice(0, page.objects.length, ...page.objects.filter(o => !selected.has(o.id)));if(Array.isArray(page.layers))page.layers.forEach(layer=>layer.objectIds=layer.objectIds.filter(id=>!selected.has(id)));for(const o of page.objects)if(o.type==='connector')for(const e of [o.from,o.to])if(selected.has(e.objectId))e.objectId=null;pruneAnimations(page);reconcileLayers(page); return Array.from(selected); }
  function reorderObjects(page, ids, mode) {
    const selected = new Set(expandSelection(page, ids)); assertLayerMutable(page,selected);
    if (!['front', 'back', 'forward', 'backward'].includes(mode)) throw new RangeError('Unknown reorder mode: ' + mode);
    const source = page.objects;
    const reorder = list => { if (mode === 'front') return list.filter(o => !selected.has(o.id)).concat(list.filter(o => selected.has(o.id))); if (mode === 'back') return list.filter(o => selected.has(o.id)).concat(list.filter(o => !selected.has(o.id))); if (mode === 'forward') { for (let i = list.length - 2; i >= 0; i--) if (selected.has(list[i].id) && !selected.has(list[i + 1].id)) [list[i], list[i + 1]] = [list[i + 1], list[i]]; } else { for (let i = 1; i < list.length; i++) if (selected.has(list[i].id) && !selected.has(list[i - 1].id)) [list[i], list[i - 1]] = [list[i - 1], list[i]]; } return list; };
    if (Array.isArray(page.layers)) { const byId = new Map(source.map(o=>[o.id,o])); page.layers.forEach(layer => { const next = reorder(layer.objectIds.map(id=>byId.get(id))); layer.objectIds = next.map(o=>o.id); }); reconcileLayers(page); } else page.objects = reorder(source);
  ...
  function duplicatePage(doc, pageId) { const page = docAndPage(doc, pageId); const copy = clone(page); copy.id = uid('page'); copy.name = page.name + ' のコピー'; const objectIds = new Map(); const groups = new Map(); copy.objects.forEach(o => { objectIds.set(o.id, uid('object')); if (o.group !== null && !groups.has(o.group)) groups.set(o.group, uid('group')); }); copy.objects.forEach(o => { o.id = objectIds.get(o.id); if (o.group !== null) o.group = groups.get(o.group);remapConnections(o,objectIds); });if(Array.isArray(copy.layers))copy.layers.forEach(layer=>{layer.id=uid('layer');layer.objectIds=layer.objectIds.map(objectId=>objectIds.get(objectId));});if(Array.isArray(copy.animations))copy.animations.forEach(animation=>{animation.id=uid('animation');animation.targets=animation.targets.map(target=>objectIds.get(target));}); doc.pages.splice(doc.pages.indexOf(page) + 1, 0, copy); return copy.id; }
