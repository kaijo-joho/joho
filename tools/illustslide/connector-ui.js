/* Connection gestures and the selection-only routing dialog. */
(function(root){
  'use strict';
  root.IlapoConnectorUI={create:function(ctx){
    ...
    function dialog(){
      ...
      const ends=['from','to'].map((key,i)=>`<fieldset><legend>${i?'終点':'始点'}</legend><label>接続先<select id="connection-${key}-target">${choices([['','接続なし（座標指定）'],...targets.map(v=>[v.id,v.name||'図形'])],o[key].objectId||'')}</select></label><div class="fields"><label>接続位置<select id="connection-${key}-port">${choices(ports,o[key].port)}</select></label><label>辺の位置（0〜1）<input id="connection-${key}-ratio" type="number" min="0" max="1" step="any" value="${o[key].ratio}"></label><label>X（px）<input id="connection-${key}-x" type="number" step="any" value="${ctx.round(o[key].x)}"></label><label>Y（px）<input id="connection-${key}-y" type="number" step="any" value="${ctx.round(o[key].y)}"></label></div></fieldset>`).join('');
      const arrows=[['none','なし'],['triangle','三角'],['open','開いた矢印']];
      ctx.showDialog('接続・経路',`<label>経路<select id="connection-route">${choices([['straight','直線・折れ線'],['orthogonal','直角']],o.route)}</select></label><div class="fields"><label>始点の矢印<select id="connection-start">${choices(arrows,o.startArrow)}</select></label><label>終点の矢印<select id="connection-end">${choices(arrows,o.endArrow)}</select></label></div><label>ラベル<textarea id="connection-label" maxlength="2000">${esc(o.label)}</textarea></label><div class="fields"><label>ラベルの横ずれ（px）<input id="connection-label-x" type="number" step="any" value="${o.labelOffset.x}"></label><label>ラベルの縦ずれ（px）<input id="connection-label-y" type="number" step="any" value="${o.labelOffset.y}"></label></div>${ends}<h3>折れ曲がり点</h3><div id="connection-points"></div><button id="connection-add-point" type="button">＋ 点を追加</button><p class="muted">点は始点から順番に通ります。キャンバスでは点をドラッグ、線をダブルクリックして追加できます。接続先を動かすと端点が追従します。</p>`,'適用',()=>{
        ...
        const checked=C.validateObject(o);ctx.changePage(p=>p.objects[p.objects.findIndex(v=>v.id===o.id)]=checked);active=null;
      });
      function readRows(){o.waypoints.forEach((p,i)=>{p.x=Number($('connection-point-x-'+i).value);p.y=Number($('connection-point-y-'+i).value);});}
      function rows(){
        $('connection-points').innerHTML=o.waypoints.map((p,i)=>`<div class="connection-point-row"><span>${i+1}</span><label>X<input id="connection-point-x-${i}" aria-label="点${i+1}のX" type="number" step="any" value="${p.x}" required></label><label>Y<input id="connection-point-y-${i}" aria-label="点${i+1}のY" type="number" step="any" value="${p.y}" required></label><button type="button" data-connection-remove="${i}" aria-label="点${i+1}を削除">×</button></div>`).join('');
