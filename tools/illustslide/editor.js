...
(function () {
  ...
  let pathUI,connectionUI,animationUI,pagesUI,objectsUI,assetsUI,exportUI,textUI,outlineUI,library,inspector,inspectorPreview=null,revision=0;
  ...
  function change(fn, group) {
    const before=snapshot(), previous=doc();
    try { history.change(d=>{fn(d);d.pages.forEach(p=>{L.reconcile(p,previous.pages.find(v=>v.id===p.id),activeLayer(p));K.sync(p);});},{group:group??inspector?.changeGroup}); pageId=page().id; selected=selected.filter(id=>page().objects.some(o=>o.id===id&&isVisible(o))); if(before!==snapshot()){inlinePlayback?.stop();revision++;queueSave();} render(); return before!==snapshot(); }
