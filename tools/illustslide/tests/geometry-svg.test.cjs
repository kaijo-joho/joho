...
(async () => {
  ...
  const report = await page.evaluate(() => {
    ...
    const mixedText = { id: 'mixed-text', type: 'text', name: '混在書式', group: null, locked: false, matrix: [1, 0, 0, 1, 0, 0], style: { ...style, bold: false, italic: false, fontSize: 28 }, x: 30, y: 110, runs: [{ text: '通常 ', script: 'normal' }, { text: '太字', script: 'normal', bold: true }, { text: ' 斜体', script: 'normal', italic: true }] };
    const b = IlapoGeometry.bounds(curved);
    const textBox = IlapoGeometry.bounds(text);
    const mixedSvg = IlapoSVG.exportPage({ id: 'mixed', name: '混在書式', board: { width: 400, height: 180, unit: 'px', infinite: false }, objects: [mixedText] });
    const mixedRoot = document.createElement('div');
    mixedRoot.style.cssText = 'position:absolute;left:-10000px;top:-10000px;opacity:0;overflow:visible';
    mixedRoot.innerHTML = mixedSvg;
    document.body.appendChild(mixedRoot);
    const mixedNode = mixedRoot.querySelector('text');
    const mixedBox = mixedNode.getBBox(), mixedNative = { x: mixedBox.x, y: mixedBox.y, width: mixedBox.width, height: mixedBox.height };
    const mixedGeometry = IlapoGeometry.bounds(mixedText);
    mixedRoot.remove();
    ...
    return { b, textBox, mixedNative, mixedGeometry, before, after, svg, imported, zipLength: zip.length, resumed, beforeD, nativeSvg, selectionRoundTrip, blocked, ordered, rootBlocked, shiftedViewBox, named };
  });
  ...
  assert(report.mixedNative.width > 0 && report.mixedNative.height > 0, 'mixed text SVG BBox is measurable');
  for (const key of ['x', 'y', 'width', 'height']) assert.ok(Math.abs(report.mixedNative[key] - report.mixedGeometry[key]) < 0.01, `mixed text ${key} follows SVG書式`);
