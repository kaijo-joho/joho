...
async function run() {
  ...
  try {
    ...
    await page.locator('#inspector-toggle').click(); await page.locator('[data-inspector-section=board]').click(); await page.locator('#board-preset').selectOption('18'); await inspectorSubmit(page); doc = await documentOf(page); let currentId = await page.evaluate(() => IlapoEditor.getState().pageId); let current = doc.pages.find(p => p.id === currentId); assert.equal(current.board.width, 18); await inspectorClose(page);
    await page.locator('#inspector-toggle').click(); await page.locator('[data-inspector-section=board]').click(); await page.locator('#board-preset').selectOption('a4'); await inspectorSubmit(page); doc = await documentOf(page); currentId = await page.evaluate(() => IlapoEditor.getState().pageId); current = doc.pages.find(p => p.id === currentId); assert(Math.abs(current.board.width - 210 * 96 / 25.4) < 1e-6, 'A4 preset uses CSS-pixel width'); await inspectorClose(page);
