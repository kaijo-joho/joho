...
async function run(){
  ...
  try{
    ...
    await page.locator('.side-tab [data-action=export-toggle]').click();await page.locator('[data-export-mode="all"]').click();await page.locator('#export-format').selectOption('html');const downloaded=page.waitForEvent('download');await page.locator('#export-panel [data-action=export-save]').click();const download=await downloaded;assert(download.suggestedFilename().endsWith('.play.html'));
