...
async function run(){
  ...
  try{
    ...
    await pick(example.pages[0].objects.at(-1).id);await page.locator('#canvas').focus();await page.keyboard.press('a');await page.locator('[data-menu=view]').click();await page.locator('#command-menu [data-action=view-dialog]').click();await page.locator('#view-theme').selectOption('dark');await submit();await page.setViewportSize({width:390,height:844});await page.locator('#connection-options').click();await page.screenshot({path:'/private/tmp/illustslide-stage3-dark-dialog.png'});assert(await page.evaluate(()=>document.getElementById('dialog').scrollWidth<=innerWidth));
