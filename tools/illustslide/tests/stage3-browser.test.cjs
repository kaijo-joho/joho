...
async function run(){
  ...
  try{
    ...
    await page.locator('[data-action=pages]').first().click();await page.locator('[data-page-command=duplicate]').click();await page.locator('#inspector-close').click();const beforePresent=await read();await startPresentation(page);
    ...
    for(const width of [736,390]){await page.setViewportSize({width,height:840});await sleep(80);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.locator('#assets-toggle').click();assert(await page.locator('[data-insert-icon=person]').isVisible());await inspectorClose();await page.screenshot({path:`/private/tmp/illustslide-stage3-${width}.png`});}
    ...
    await touch.locator('#file-input').setInputFiles({name:'route.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(routed))});await touch.waitForFunction(id=>IlapoEditor.getDocument().id===id,routed.id);await touch.locator('#objects-toggle').click();await touch.locator(`[data-pick-object="${connection.id}"]`).click();await touch.locator("#inspector-close").click();await sleep(60);
