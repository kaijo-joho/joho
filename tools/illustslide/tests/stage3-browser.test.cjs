...
async function run(){
  ...
  const pick=async id=>{await page.locator('#objects-toggle').click();await page.locator(`[data-pick-object="${id}"]`).click();await page.locator("#inspector-close").click();await sleep(50);};
  ...
  const inspectorSubmit=async()=>{await page.locator('#inspector-submit').click();await page.waitForFunction(()=>{const panel=document.getElementById('inspector-panel');return panel&&!panel.hidden&&!document.getElementById('dialog').open;});await sleep(40);};
  ...
  try{
    ...
    await pick(connection.id);await page.locator('#canvas').focus();await page.keyboard.press('a');await page.locator('#connection-options').click(); assert(await page.locator('#inspector-panel').isVisible(),'connection settings use the non-modal inspector'); assert(await page.locator('#canvas').isVisible(),'canvas remains visible while connection settings are open');
    ...
    await page.locator('#connection-add-point').click();await page.locator('#connection-point-x-0').fill('280');await page.locator('#connection-point-y-0').fill('270');await page.locator('#connection-add-point').click();await page.locator('#connection-point-x-1').fill('360');await page.locator('#connection-point-y-1').fill('300');await inspectorSubmit();await inspectorClose();
    ...
    await page.locator('#canvas').focus();await page.keyboard.press('Meta+a');await edit('component-save');await page.locator('#component-name').fill('送受信 <部品>');await page.locator('#components-register').click();await sleep(50);
    ...
    await page.locator('[data-insert-icon=pc]').click();await sleep(50);await inspectorClose();assert.equal((await read()).pages[0].objects.at(-1).type,'path');
    ...
    await page.locator('#image-options').click();assert(await page.locator('#inspector-panel').isVisible(),'image settings use the non-modal inspector');await page.locator('#image-locked').uncheck();await page.locator('#image-output').check();await page.locator('#image-opacity').fill('100');await inspectorSubmit();await inspectorClose();assert((await page.evaluate(()=>IlapoSVG.exportPage(IlapoEditor.getDocument().pages[0]))).includes('<image'));
    ...
    await page.locator('[data-action=pages]').first().click();await page.locator('[data-page-command=duplicate]').click();await page.locator('#inspector-close').click();const beforePresent=await read();await page.locator('#present-button').click();
    ...
    await touch.locator('#file-input').setInputFiles({name:'route.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(routed))});await touch.waitForFunction(id=>IlapoEditor.getDocument().id===id,routed.id);await touch.locator('[data-menu=more]').click();await touch.locator('#command-menu [data-action=objects]').click();await touch.locator(`[data-pick-object="${connection.id}"]`).click();await touch.locator("#inspector-close").click();await sleep(60);
    ...
    await load(example);await page.screenshot({path:'/private/tmp/illustslide-stage3-example.png'});await page.locator('#present-button').click();await page.screenshot({path:'/private/tmp/illustslide-stage3-presentation.png'});
    ...
    await pick(example.pages[0].objects.at(-1).id);await page.locator('#canvas').focus();await page.keyboard.press('a');await page.locator('[data-menu=view]').click();await page.locator('#command-menu [data-action=view-dialog]').click();await page.locator('#view-theme').selectOption('dark');await inspectorSubmit();await inspectorClose();await page.setViewportSize({width:390,height:844});await page.locator('#connection-options').click();await page.screenshot({path:'/private/tmp/illustslide-stage3-dark-dialog.png'});assert(await page.evaluate(()=>document.getElementById('inspector-panel').scrollWidth<=innerWidth));assert(await page.locator('#canvas').isVisible());await inspectorClose();
