...
(async()=>{
  ...
  try{
    ...
    await page.locator('#connection-options').click();await page.locator('#connection-to-ratio').fill('0.75');await page.locator('#inspector-submit').click();await settle(page);near(line(await read(page)).to.ratio,.75);await page.locator('#inspector-close').click();
    for(const width of [736,390]){await page.setViewportSize({width,height:844});await settle(page);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.locator('[data-menu=more]').click();assert(await page.locator('#command-menu [data-tool=connector-orthogonal]').isVisible());await page.keyboard.press('Escape');}
    await page.locator('#inspector-toggle').click();await page.locator('[data-inspector-section=view]').click();await page.locator('#view-theme').selectOption('dark');await page.locator('#view-size').selectOption('xlarge');await page.locator('#inspector-submit').click();await settle(page);await page.locator('#inspector-close').click();await settle(page);await page.screenshot({path:'/private/tmp/illustslide-elbow-390px.png'});
