...
(async()=>{
  ...
  try{
    ...
    await rail(page,'objects');await action('layer-visible').click();await settle(page);await action('layer-back').click();await settle(page);assert.equal((await read(page)).pages[0].layers[0].id,layerId);await history(page);assert.equal((await read(page)).pages[0].layers[1].id,layerId);
