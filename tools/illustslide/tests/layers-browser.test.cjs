...
async function dragObject(page,source,target,before=true){await source.scrollIntoViewIfNeeded();await target.scrollIntoViewIfNeeded();const from=await source.boundingBox(),to=await target.boundingBox();assert(from&&to);await page.mouse.move(from.x+from.width/2,from.y+from.height/2);await page.mouse.down();await page.mouse.move(to.x+to.width/2,to.y+to.height*(before ? .25 : .7),{steps:5});assert.equal(await page.locator(before?'.drop-before':'.drop-after').count(),1,'レイヤー間の挿入位置を表示する');await page.mouse.up();await settle(page);}
...
(async()=>{
  ...
  try{
    ...
    assert.deepEqual(await page.locator('.side-tab button').allTextContents(),['ページ','レイヤー','動きと再生順序','部品','表示と吸着','用紙サイズ','書き出し']);
    ...
    const action=async command=>{await settle(page);const menu=page.locator(`[data-layer-menu="${layerId}"]`);if(!(await menu.isVisible())){await page.locator(`[data-layer-more="${layerId}"]`).click();await settle(page);}return page.locator(`[data-object-command="${command}"][data-object-id="${layerId}"]`);};
    await (await action('layer-rename')).click();await page.locator('#layer-name').fill('イラスト');await page.locator('#dialog-form button[type=submit]').click();await settle(page);assert.equal((await read(page)).pages[0].layers[1].name,'イラスト');
    ...
    await dragObject(page,page.locator('[data-object-row="red"] [data-object-drag]'),page.locator('[data-object-row="blue"]'),true);
    doc=await read(page);assert.deepEqual(doc.pages[0].layers[0].objectIds,['blue','red'],'別レイヤーへの前面ドロップは対象行の後ろへ図形を移す');assert.deepEqual(doc.pages[0].layers[1].objectIds,[]);
    await history(page);assert.deepEqual((await read(page)).pages[0].layers[1].objectIds,['red'],'レイヤー間のドラッグは1回のUndoで戻る');await page.locator('[data-object-pick=red]').click();
    ...
    await rail(page,'objects');await (await action('layer-lock')).click();await settle(page);const locked=await read(page);
    ...
    await rail(page,'objects');await (await action('layer-lock')).click();await (await action('layer-visible')).click();await settle(page);
