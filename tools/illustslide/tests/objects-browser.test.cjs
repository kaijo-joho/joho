...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('[data-pick-object="grouped"]').getAttribute('aria-label'), /固定.*グループ/);
    ...
    assert.equal(await page.locator('[data-pick-object="back"]').getAttribute('aria-selected'), 'true');
    ...
    assert.deepEqual(new Set(await page.evaluate(() => IlapoEditor.getSelection())), new Set(['front', 'grouped']));
    await page.keyboard.press('Home'); await settle(page);
    assert.equal(await page.evaluate(() => document.activeElement.dataset.objectPick), 'front');
    ...
    await page.locator('#objects-toggle').click(); await page.locator('[data-pick-object="back"]').click(); await settle(page);
