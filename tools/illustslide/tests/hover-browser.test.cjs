...
async function run(){
  ...
  try {
    ...
    await page.locator('#inspector-toggle').click();const topTabs=await page.locator('#inspector-tabs [data-inspector-section]').evaluateAll(nodes=>nodes.map(node=>node.dataset.inspectorSection));assert(!topTabs.some(section=>['pages','objects','assets'].includes(section)));assert.equal(await page.locator('#pages-toggle').isVisible(),true);assert.equal(await page.locator('#objects-toggle').isVisible(),true);assert.equal(await page.locator('#assets-toggle').isVisible(),true);
