...
assert.equal(A.icons().length,10);const icon=A.instantiateIcon('person',{x:12,y:8,size:40});assert.equal(icon[0].type,'path');assert.deepEqual(icon[0].matrix.slice(4),[12,8]);
const saved=lib.save('人物',icon);assert.equal(lib.list()[0].name,'人物');const made=lib.instantiate(saved.id,{x:100,y:30,size:20});assert.equal(made[0].group!==null,true);assert.equal(made[0].locked,false);assert.equal(made[0].matrix[4],100);assert.equal(Math.abs(made[0].matrix[0]),2);
