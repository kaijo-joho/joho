...
async function transform(width,scale){await page.locator('#selection-transform').click();await page.locator('#command-menu [data-action="transform"]').click();await page.locator('#transform-width').fill(String(width));if(scale)await page.locator('#scale-stroke').check();await settle(page);}
