...
(async () => {
  ...
  try {
    ...
    const report = await page.evaluate(async () => {
      ...
      return { fonts: IlapoTextOutline.fonts(), before: before === JSON.stringify(direct), unprepared, validated: !!IlapoCore.validateObject(sans.objects[0]), sans, serif, shapeOutline, connectorOutline, missing, empty, woffGlyphs, otfGlyphs, woffAdvance: woff.getAdvanceWidth(probe, 32), otfAdvance: otf && otf.getAdvanceWidth(probe, 32), box: { x: box.x, y: box.y, width: box.width, height: box.height } };
    });
