...
(function (root, factory) {
  ...
  function cloneRuns(runs) { return runs.map(function (run) { var out={ text: String(run.text == null ? '' : run.text), script: script(run.script) }; if(run.bold!==undefined)out.bold=!!run.bold;if(run.italic!==undefined)out.italic=!!run.italic;if(run.fill!==undefined)out.fill=run.fill;return out; }); }
  function runStyle(base, run) { return Object.assign({}, base, run && run.bold !== undefined ? {bold:!!run.bold} : null, run && run.italic !== undefined ? {italic:!!run.italic} : null, run && run.fill !== undefined ? {fill:run.fill} : null); }
  function runKey(run) { return run.script+'|'+(run.bold===undefined?'':run.bold?'1':'0')+'|'+(run.italic===undefined?'':run.italic?'1':'0')+'|'+(run.fill===undefined?'':run.fill); }
  ...
  function units(runs) {
    var text = '', spans = [], at = 0; runs.forEach(function (run) { var value = String(run.text); if (value) { spans.push(Object.assign({ start: at, end: at + value.length, script: script(run.script) },cloneRuns([run])[0])); text += value; at += value.length; } });
    var span = 0; return { text: text, list: graphemes(text).map(function (part) { while (span + 1 < spans.length && part.index >= spans[span].end) span++; var source=spans[span]||{script:'normal'},out={ text: part.text, start: part.index, end: part.index + part.text.length, script: source.script };['bold','italic','fill'].forEach(function(prop){if(source[prop]!==undefined)out[prop]=source[prop];});return out; }) };
  }
  ...
  function append(line, part, s, measure) { part.forEach(function (unit) { var last = line.runs[line.runs.length - 1], effective=runStyle(s,unit), width = metric(unit.text, effective, unit.script, measure).width, key=runKey(unit); if (last && last.key === key) last.parts.push(unit.text); else { var next={ script: unit.script, parts: [unit.text], key:key };['bold','italic','fill'].forEach(function(prop){if(unit[prop]!==undefined)next[prop]=unit[prop];});line.runs.push(next); } line.rough += width; }); }
  function tokenWidth(part, s, measure) { return part.reduce(function (sum, unit) { return sum + metric(unit.text, runStyle(s,unit), unit.script, measure).width; }, 0); }
  function put(lines, line, token, width, s, measure) {
    ...
    if (token.word) { if (line.runs.length) lines.push(line); line = blank(); token.units.forEach(function (unit) { var unitWidth = metric(unit.text, runStyle(s, unit), unit.script, measure).width; if (line.rough && line.rough + unitWidth > width) { lines.push(line); line = blank(); } append(line, [unit], s, measure); }); return line; }
  ...
  function finished(line, s, measure) {
    var cursor = 0, ink = null, visual = null, runs = line.runs.map(function (run) { var text = run.parts.join(''), effective=runStyle(s,run), m = metric(text, effective, run.script, measure), base = baselineShift(s, run.script), bounds = { left: cursor - m.left, right: cursor + m.right, top: base - m.ascent, bottom: base + m.descent }, safe = { left: cursor - m.left - .5, right: cursor + m.right + .5, top: base - Math.max(m.ascent, m.fontAscent) - .5, bottom: base + Math.max(m.descent, m.fontDescent) + .5 }; cursor += m.width; if (text) { ink = ink ? { left: Math.min(ink.left, bounds.left), right: Math.max(ink.right, bounds.right), top: Math.min(ink.top, bounds.top), bottom: Math.max(ink.bottom, bounds.bottom) } : bounds; visual = visual ? { left: Math.min(visual.left, safe.left), right: Math.max(visual.right, safe.right), top: Math.min(visual.top, safe.top), bottom: Math.max(visual.bottom, safe.bottom) } : safe; } var out={text:text,script:run.script};['bold','italic','fill'].forEach(function(prop){if(run[prop]!==undefined)out[prop]=run[prop];});return out; }); return { width: cursor, runs: runs, ink: ink, visual: visual };
  }
