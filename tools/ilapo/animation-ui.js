/* Animation commands live in selection and page popups, never a permanent panel. */
(function (root) {
  'use strict';
  ...
  function create(ctx) {
    ...
    function list() {
      ...
      ctx.showDialog('動きと再生順序', `<p class="muted">${esc(page.name)} · クリック ${plan.steps}回。最初の「同時」「後」は、ページを開くと自動で始まります。</p><div class="animation-list">${animations.map((a, i) => {
    ...
    function edit(id) {
      ...
      function read() {
        if (!$('dialog-form').reportValidity()) throw Error('入力値を確認してください。');
        const effect = $('animation-effect').value;
