...
<body><main><h1>${esc(checked.name)}</h1><p>クリック・Space・→で次へ、←で前へ、Rでこのページをやり直します。Escapeで終了します。</p><button type="button" id="restart">先頭から再生</button><p id="error" role="alert" hidden></p><p>イラポで作成 · このファイルだけで再生できます。</p><details><summary>再生ライブラリの利用条件</summary><pre>${license}</pre></details></main><noscript>再生にはJavaScriptを有効にしてください。</noscript>
