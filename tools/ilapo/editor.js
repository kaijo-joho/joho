...
(function () {
  ...
  let settings={theme:'auto',size:'standard',grid:true,snap:false,gridStep:20};
  ...
  function change(fn) {
    ...
    try { history.change(fn); pageId=page().id; selected=selected.filter(id=>page().objects.some(o=>o.id===id)); if(before!==snapshot()){edited=true;queueSave();} render(); return before!==snapshot(); }
  ...
  function renderNow() {
    ...
    $('canvas').dataset.tool=tool==='pan'||space?'pan':tool==='select'?'select':'draw';
    ...
    $('artwork').innerHTML=p.objects.map(o=>`<g data-object="${esc(o.id)}" aria-label="${esc(o.name)}">${S.objectMarkup(o)}</g>`).join('');
    $('welcome').hidden=!!p.objects.length||!!drag; $('selection-bar').hidden=!selected.length||!!drag;
    $('selection-count').textContent=selected.length+'個'; $('document-title').textContent=doc().name; document.title=doc().name+' — イラポ';
    ...
    $('hint').textContent=tool==='select'?(selected.length>1?'整列・サイズの基準は最初に選んだ図形 · Space＋ドラッグで移動':'Shiftで追加選択 · Space＋ドラッグで移動'):tool==='pan'?'ドラッグして表示を移動':`${names[tool]}：クリックで配置 · ドラッグで大きさを指定`;
    renderSelection();
  }
  ...
  function setTool(value){cancelDrag();tool=value;hideMenu();render();$('canvas').focus();}
  function cancelDrag(){if(drag?.originalSelection)selected=drag.originalSelection;preview=null;drag=null;render();}
  ...
  $('canvas').addEventListener('pointerdown',event=>{
    ...
    else if(tool==='text'){textDialog(null,point);tool='select';render();return;}
  ...
  $('canvas').addEventListener('pointerup',event=>{
    ...
    if(action.kind==='draw'){
      const object=createShape(tool,action.start,action.end,!action.moved);changePage(p=>p.objects.push(object));selected=[object.id];tool='select';
  ...
  $('canvas').addEventListener('dblclick',event=>{const id=event.target.closest('[data-object]')?.dataset.object;const o=page().objects.find(o=>o.id===id);if(o?.type==='text'&&!o.locked)textDialog(o);});
  ...
  $('command-menu').addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();hideMenu();menuOpener?.focus();}});
  let dialogOpener,dialogCallback;
  function closeDialog(){if($('dialog').open)$('dialog').close();dialogOpener?.focus();}
  function showDialog(title,html,label,callback){hideMenu();if($('dialog').open)$('dialog').close();dialogOpener=document.activeElement;$('dialog-title').textContent=title;$('dialog-body').innerHTML=html;$('dialog-error').hidden=true;$('dialog-submit').textContent=label||'適用';$('dialog-submit').hidden=!label;dialogCallback=callback;$('dialog').showModal();}
  ...
  function replaceDocument(value){history.replace(value);pageId=doc().pages[0].id;selected=[];edited=false;saveFingerprint=snapshot();clearTimeout(saveTimer);localAuto?.stop();$('save-status').textContent='読み込みました';fit();}
  ...
  function styleDialog(){
    if(!editable())return;const objects=page().objects.filter(o=>selected.includes(o.id)),first=objects[0].style,patch={};let channel='fill';
    ...
    $('dialog-body').querySelectorAll('[data-style]').forEach(el=>{if(el.tagName==='SELECT')el.value=first[el.dataset.style];el.onchange=()=>{const key=el.dataset.style;patch[key]=el.type==='checkbox'?el.checked:el.type==='number'?Number(el.value)/(key==='opacity'?100:1):el.value;};});updateColor();
  }
  function viewDialog(){showDialog('表示設定',`<label>テーマ<select id="view-theme"><option value="auto">自動</option><option value="light">ライト</option><option value="dark">ダーク</option></select></label><label>操作部の文字サイズ<select id="view-size"><option value="standard">標準</option><option value="large">大</option><option value="xlarge">特大</option></select></label><label class="check"><input id="view-grid" type="checkbox" ${settings.grid?'checked':''}>グリッドを表示</label><label class="check"><input id="view-snap" type="checkbox" ${settings.snap?'checked':''}>移動・配置をグリッドに吸着</label><label>グリッドの間隔（px）<input id="view-step" type="number" min="0.01" max="10000" step="any" required value="${Number(settings.gridStep)||20}"></label><p class="muted">Optionを押している間は吸着を解除します。テーマを変えても作品の色は変わりません。</p>`,'適用',()=>{settings={theme:$('view-theme').value,size:$('view-size').value,grid:$('view-grid').checked,snap:$('view-snap').checked,gridStep:Number($('view-step').value)};applySettings();});$('view-theme').value=settings.theme;$('view-size').value=settings.size;}
  ...
  function copy(){if(!selected.length)return;clipboard={objects:C.clone(page().objects.filter(o=>selected.includes(o.id))),count:0};toast('図形をコピーしました。別のページにも貼り付けられます。');}
  function paste(){if(!clipboard)return;const temp=C.createPage('コピー',page().board);temp.objects=C.clone(clipboard.objects);const offset=++clipboard.count*standardSize()*.12,ids=C.duplicateObjects(temp,temp.objects.map(o=>o.id),offset,offset);const copies=temp.objects.filter(o=>ids.includes(o.id));copies.forEach(o=>o.locked=false);if(changePage(p=>p.objects.push(...copies)))selected=ids;render();}
  const commands={
    ...
    'export-svg':()=>{const selectionIds=$('export-range').value==='selection'?selected:null;if(selectionIds&&!selectionIds.length){toast('書き出す図形を選択してください。');return;}const padding=Number($('export-padding').value);if(!Number.isFinite(padding)||padding<0||padding>1000){toast('余白は0〜1000pxで指定してください。');return;}download(S.exportPage(page(),{selectionIds,padding}),fileName(doc().name+'_'+page().name,'.svg'),'image/svg+xml');toast('SVGを書き出しました。');}
  };
  function execute(action){hideMenu();cancelDrag();try{if(action.startsWith('align-'))align(action.slice(6));else if(action.startsWith('order-')){if(editable())changePage(p=>C.reorderObjects(p,selected,action.slice(6)));}else commands[action]?.();}catch(error){toast(errorMessage(error));}}
  ...
  document.addEventListener('keydown',event=>{
    if(help?.root.contains(event.target)||event.isComposing||event.target.closest('input,textarea,select,[contenteditable=true]')||$('dialog').open)return;
    if(event.key==='Escape'){if(drag)cancelDrag();else if($('command-menu').matches(':popover-open')){hideMenu();menuOpener?.focus();}else selected=[];space=false;render();return;}
    ...
    else if(event.key.startsWith('Arrow')&&selected.length&&!event.target.closest('button')){event.preventDefault();if(editable()){const n=(event.shiftKey?10:1)*(page().board.width<100?.1:1);changePage(p=>C.transformObjects(p,selected,[1,0,0,1,event.key==='ArrowLeft'?-n:event.key==='ArrowRight'?n:0,event.key==='ArrowUp'?-n:event.key==='ArrowDown'?n:0]));}}
    else if(event.key==='?')help?.open();else if(key==='v')setTool('select');else if(key==='h')setTool('pan');else if(key==='t')setTool('text');else if(key==='r')setTool('rect');else if(key==='e')setTool('ellipse');
  });
  ...
  window.IlapoEditor=Object.freeze({getDocument:()=>C.clone(doc()),getSelection:()=>selected.slice(),getCamera:()=>({...camera}),getState:()=>({tool,dirty:dirty(),pageId})});
