...
(function () {
  'use strict';
  const C = window.IlapoCore, S = window.IlapoSVG, G = window.IlapoGeometry, E = window.IlapoExport;
  ...
  const names = {rect:'長方形',roundrect:'角丸',ellipse:'楕円',triangle:'三角形',pentagon:'五角形',line:'線',text:'文字',direct:'直接選択',select:'全体を選択',pan:'移動'};
  ...
  if (!C || !S || !G || !E || !window.IlapoPathEdit || !window.IlapoPathUI) { $('hint').textContent='必要なファイルを読み込めませんでした。ページを再読み込みしてください。'; return; }
  ...
  function renderNow() {
    ...
    $('hint').textContent=tool==='direct'?pathUI.hint():tool==='select'?(selected.length>1?'整列・サイズの基準は最初に選んだ図形 · Space＋ドラッグで移動':'Shiftで追加選択 · Space＋ドラッグで移動'):tool==='pan'?'ドラッグして表示を移動':`${names[tool]}：クリックで配置 · ドラッグで大きさを指定`;
    renderSelection();
  }
  ...
  function setTool(value){cancelDrag();if(tool!==value)pathUI?.reset();tool=value;hideMenu();render();$('canvas').focus();}
  ...
  $('canvas').addEventListener('pointermove',event=>{
    ...
    if(pathUI.pointerMove(event,p,drag)){render();return;}
  ...
  $('canvas').addEventListener('pointerup',event=>{
    ...
    if(pathUI.finishDrag(action,result)){render();return;}
    if(action.kind==='draw'){
  ...
  $('canvas').addEventListener('dblclick',event=>{if(tool==='direct')pathUI.doubleClick(event,world(event));const id=event.target.closest('[data-object]')?.dataset.object;const o=page().objects.find(o=>o.id===id);if(o?.type==='text'&&!o.locked)textDialog(o);});
  ...
  function replaceDocument(value){pathUI.reset();history.replace(value);pageId=doc().pages[0].id;selected=[];edited=false;saveFingerprint=snapshot();clearTimeout(saveTimer);localAuto?.stop();$('save-status').textContent='読み込みました';fit();}
  ...
  document.addEventListener('keydown',event=>{
    ...
    if(event.key==='Escape'){if(drag)cancelDrag();else if($('command-menu').matches(':popover-open')){hideMenu();menuOpener?.focus();}else {selected=[];pathUI.reset();}space=false;render();return;}
    ...
    if(pathUI.keyboard(event)){event.preventDefault();return;}
