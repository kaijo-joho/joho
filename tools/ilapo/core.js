(function (root, factory) {
  ...
  function validateDocument(input) {
    ...
    if (input.format !== 'kaijo-ilapo') fail('format is invalid'); if (![1,2].includes(input.version)) fail('version is invalid');
    ...
    let imageBytes=0;for(const page of out.pages)for(const o of page.objects){if(['image','connector'].includes(o.type))out.version=2;if(o.type==='image')imageBytes+=o.src.length;}
  ...
  function removeObjects(page, ids) { const selected = new Set(expandSelection(page, ids)); page.objects.splice(0, page.objects.length, ...page.objects.filter(o => !selected.has(o.id)));for(const o of page.objects)if(o.type==='connector')for(const e of [o.from,o.to])if(selected.has(e.objectId))e.objectId=null; return Array.from(selected); }
  ...
  function duplicatePage(doc, pageId) { const page = docAndPage(doc, pageId); const copy = clone(page); copy.id = uid('page'); copy.name = page.name + ' のコピー'; const objectIds = new Map(); const groups = new Map(); copy.objects.forEach(o => { objectIds.set(o.id, uid('object')); if (o.group !== null && !groups.has(o.group)) groups.set(o.group, uid('group')); }); copy.objects.forEach(o => { o.id = objectIds.get(o.id); if (o.group !== null) o.group = groups.get(o.group);remapConnections(o,objectIds); }); doc.pages.splice(doc.pages.indexOf(page) + 1, 0, copy); return copy.id; }
