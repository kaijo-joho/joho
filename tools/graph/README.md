...
- 接線はアイコンのホバー／クリック／タップで座標欄を開き、入力中に線と方程式をプレビューする。Enterまたは追加ボタンで登録し、名前などの詳細は登録後に編集する。Esc・外側クリックで取り消し、未確定の接線は履歴・保存・書き出しに含めない。
- 点・線分・接線・交点・文字・補助線は左パネルの直接ボタンから追加する。選択時のポップアップでは編集・色・表示・複製・削除・出典を線画アイコンと説明で操作し、一覧のダブルクリックまたは選択後のEnterでも設定を開ける。基本6色をポップアップへ常設し、残りの12色・線幅・線種、注釈文字の表示・サイズはメニュー内で直接変更する。RGB・自由な色・不透明度・細かな文字位置は「詳細」のダイアログを使う。
