...
- 注釈ラベルの表示・位置・文字サイズ、点・文字のドラッグ。接線式は左一覧で常に確認でき、図中の表示は設定で切り替える。旧文書の図中接線式は非表示で移行する。数値だけの自由点・文字位置はドラッグで更新できるが、`1/2` や係数を含む式は失わず、ダイアログで編集する。
- 点・線分・接線・交点・文字・補助線は左パネルの直接ボタンから追加する。選択時のポップアップでは編集・色・表示・複製・削除・出典を線画アイコンと説明で操作し、一覧のダブルクリックまたは選択後のEnterでも設定を開ける。18色パレット、自由な色指定、RGB、線幅・線種・不透明度を備える。
