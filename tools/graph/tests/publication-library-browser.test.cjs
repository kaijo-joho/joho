...
(async()=>{
  ...
  await bar.getByRole('button',{name:'数表・出典',exact:true}).click();const publicationCell=dialog.getByLabel('2行2列',{exact:true});await publicationCell.click();await publicationCell.press('F2');await publicationCell.fill('12');await publicationCell.press('Tab');
  ...
  await dialog.locator('.graph-table-editor__column-menu summary').click();await dialog.getByRole('button',{name:'計算列を追加',exact:true}).click();await dialog.getByRole('button',{name:'数値をCSVで保存',exact:true}).click();assert.match(await page.locator('#dialog-error').innerText(),/未適用/);await close();assert.deepEqual(await doc(),C.validateDocument(fixture));
