...
(async () => {
  ...
  const xaxis=page.locator('[data-dialog-panel=range] [data-axis="x"]');
  ...
  await dialog.getByRole('tab',{name:'軸',exact:true}).click();
  ...
  await page.locator('#dialog-cancel').click();await Toolbar.selectWorkspace(page,'main');await settle();
  ...
  await firstDate.click();await firstDate.press('F2');await firstDate.fill('2024-02-30');const beforeBad=await doc();
  ...
  await firstDate.click();await firstDate.press('F2');await firstDate.fill('2024/1/31');await submit();
  ...
  await Toolbar.openAxes(page);await xaxis.getByLabel('カテゴリの順序（1行に1項目）',{exact:true}).fill('大阪\n東京\n京都');await submit();
  ...
  await page.locator('#dialog-cancel').click();await Toolbar.openSettings(page);await page.locator('[data-theme-value="dark"]').click();await page.locator('#text-size').selectOption('largest');await page.keyboard.press('Escape');
