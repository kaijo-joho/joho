...
(async () => {
  ...
  assert.equal((await doc()).version, 14);
  ...
  await page.waitForFunction(()=>GraphEditor.getDocument().annotations.length===0);await settle();assert.equal((await doc()).version,14);assert.deepEqual((await doc()).series,legacy.series);
