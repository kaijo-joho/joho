/* Chrome regression for curve types, dependent annotations and version 2 documents. */
...
(async () => {
  ...
  async function addAnnotation(name) { await page.locator('#add-annotation').click(); await page.getByRole('button', { name: name + 'を追加', exact: true }).click(); }
  async function more(action) { await page.locator('#selection-toolbar summary').click(); await page.locator('#selection-toolbar').getByRole('button', { name: action, exact: true }).click(); }
  ...
  assert.equal((await doc()).version, 2);
  ...
  await page.locator('#parameter-list button').click(); await page.getByLabel('a の数値', { exact: true }).fill('2'); await page.getByLabel('a の数値', { exact: true }).press('Tab'); await settle();
  ...
  await page.getByRole('button', { name: '色・線', exact: true }).click(); await page.getByLabel('色 #0891b2', { exact: true }).click(); await field('R', 10); await field('G', 20); await field('B', 30); await submit();
  ...
  await addAnnotation('交点'); await field('名前', '交点A'); await submit();
  ...
  assert(await page.locator('#add-annotation').isDisabled()); assert(await page.locator('#other-curves').isHidden());
  ...
  const serialized = JSON.parse(fs.readFileSync(await file.path(), 'utf8')); assert.equal(serialized.version, 2); assert.equal(serialized.annotations.length, 5);
  ...
  const legacy = structuredClone(serialized);legacy.version=1;delete legacy.annotations;legacy.series=legacy.series.filter(s=>s.kind==='function');
  await page.setInputFiles('#file-input',{name:'old-v1.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(legacy))});
  await page.waitForFunction(()=>GraphEditor.getDocument().annotations.length===0);await settle();assert.equal((await doc()).version,2);assert.deepEqual((await doc()).series,legacy.series);
