...
(async () => {
  ...
  assert.equal((await doc()).version, 11);
  ...
  const serialized = JSON.parse(fs.readFileSync(await file.path(), 'utf8')); assert.equal(serialized.version, 11); assert.equal(serialized.annotations.length, 5);
  ...
  await page.waitForFunction(()=>GraphEditor.getDocument().annotations.length===0);await settle();assert.equal((await doc()).version,11);assert.deepEqual((await doc()).series,legacy.series);
