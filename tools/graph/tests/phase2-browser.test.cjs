...
(async () => {
  ...
  assert.equal((await doc()).version, 12);
  ...
  const serialized = JSON.parse(fs.readFileSync(await file.path(), 'utf8')); assert.equal(serialized.version, 12); assert.equal(serialized.annotations.length, 5);
  ...
  const legacy = structuredClone(serialized);legacy.version=1;delete legacy.charts;delete legacy.comparison;delete legacy.presentation;delete legacy.output;delete legacy.annotations;legacy.series=legacy.series.filter(s=>s.kind==='function');for(const axis of Object.values(legacy.axes)){delete axis.symbol;delete axis.ticks;delete axis.type;delete axis.categories;}
  await page.setInputFiles('#file-input',{name:'old-v1.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(legacy))});
  await page.waitForFunction(()=>GraphEditor.getDocument().annotations.length===0);await settle();assert.equal((await doc()).version,12);assert.deepEqual((await doc()).series,legacy.series);
