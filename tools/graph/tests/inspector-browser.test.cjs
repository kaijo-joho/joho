...
(async () => {
  ...
  await bar.getByRole('button',{name:'複製',exact:true}).click();await settle();
  const second=(await doc()).series[1].id;
  await more('series',first).click();assert(await menu.isVisible());assert.equal(await dialog.isVisible(),false);
  assert(await menu.getByRole('menuitem',{name:'先頭へ',exact:true}).isDisabled());
  await menu.getByRole('menuitem',{name:'末尾へ',exact:true}).click();await settle();
  assert.deepEqual((await doc()).series.map(s=>s.id),[second,first]);
  assert.equal(await page.evaluate(()=>document.activeElement.dataset.objectDetails),'series:'+first);
  ...
  await page.mouse.move(380,840);await page.locator('#list-toggle').tap();if(await page.locator('#series-add-panel').isVisible())await page.keyboard.press('Escape');await more('series',first).tap();await menu.getByRole('menuitem',{name:'末尾へ',exact:true}).tap();
