...
(async()=>{
  ...
  const chart=Charts.create('matrix',{seriesId:series.id,columns:[0,1,2],name:'実験の行列'});chart.id='matrix';fixture.charts=[chart];fixture.comparison={columns:1,items:['main',chart.id]};await importDoc(fixture);await Toolbar.selectWorkspace(page,chart.id);await settle();
  ...
  await page.getByRole('button',{name:'この行を数表で編集',exact:true}).click();const matrixCell=dialog.getByLabel('4行2列',{exact:true});await matrixCell.click();await matrixCell.press('F2');await matrixCell.fill('9');await matrixCell.press('Tab');await submit();assert.equal((await state()).workspaceView,'matrix');assert.equal((await doc()).series[0].dataTable.rows[3][1],9);
