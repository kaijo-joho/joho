...
(async () => {
  ...
  assert.equal(doc.version, 11); assert.equal(doc.series.length, initialSeries + 1); assert.equal(doc.series.at(-1).rows.length, 12); assert.equal(doc.series.at(-1).dataTable.columns.length, 5);
