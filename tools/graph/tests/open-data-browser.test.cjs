...
(async () => {
  ...
  assert.equal(doc.version, 10); assert.equal(doc.series.length, initialSeries + 1); assert.equal(doc.series.at(-1).rows.length, 12); assert.equal(doc.series.at(-1).dataTable.columns.length, 5);
  ...
  assert.equal(await page.getByLabel('日付の変換', {exact:true}).isVisible(), true);
  await page.getByLabel('日付の変換', {exact:true}).selectOption('year'); await page.locator('[data-import-summary]').waitFor();
  await submit(); doc = await page.evaluate(() => GraphEditor.getDocument());
