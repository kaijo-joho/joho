...
{
 const d=doc(),r=regression(d),clean=C.validateDocument(d);assert.equal(clean.version,8);assert.equal(clean.series[0].errorBars.y[1],.1);let out=A.evaluate(clean.annotations[0],clean);assert.equal(out.warning,'');assert(out.segments.length);clean.series[0].rows[2]=[2,9];out=A.evaluate(clean.annotations[0],clean);assert.equal(out.fit.coefficients[1],4);
}
{
...
{
 const d=doc();d.version=6;delete d.presentation;delete d.output;delete d.series[0].errorBars;delete d.series[0].interpolation;const migrated=C.validateDocument(d);assert.equal(migrated.version,8);assert.deepStrictEqual(migrated.series[0].errorBars,{x:[],y:[]});
...
}
{
 ...
 for(const version of [1,2,3,4,5,6]){const d=doc();d.version=version;delete d.presentation;delete d.output;delete d.series[0].errorBars;delete d.series[0].interpolation;if(version<3)for(const a of Object.values(d.axes)){delete a.symbol;delete a.ticks;}if(version===1)delete d.annotations;const clean=C.validateDocument(d);assert.equal(clean.version,8);assert.deepStrictEqual(clean.series[0].rows,d.series[0].rows);assert.deepStrictEqual(clean.series[0].style,d.series[0].style);}
