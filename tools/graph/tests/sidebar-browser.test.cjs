...
(async () => {
  ...
  await settle();
  const series = page.locator('#series-add-toggle'), ann = page.locator('#annotation-add-toggle');
  const sp = page.locator('#series-add-panel'), ap = page.locator('#annotation-add-panel');
  ...
  assert(await sp.isHidden()); assert(await ap.isHidden());
  ...
  await ann.click(); assert(await ap.isVisible()); assert(await sp.isHidden(), 'add panels are exclusive');
  await ann.click(); assert(await ap.isHidden(), 'click toggles a pinned panel closed');
  await series.click(); await page.mouse.click(1200, 800); await sp.waitFor({ state: 'hidden' });
  ...
  await ann.click(); assert(await page.locator('#add-point').isDisabled()); assert(await page.locator('#annotation-mode-note').isVisible());
  ...
  await ann.tap(); assert(await ap.isVisible()); assert(await inViewport(ap));
  await page.locator('#add-point').tap(); await page.locator('#editor-dialog').waitFor({ state: 'visible' }); await page.locator('#dialog-cancel').tap(); assert(await focused(ann));
