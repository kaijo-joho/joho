...
}
{
  ...
  const migrated = valid(old); assert.equal(migrated.version, 11); assert.deepEqual(migrated.charts, []); assert.deepEqual(migrated.comparison, { columns: 2, items: ['main'] });
