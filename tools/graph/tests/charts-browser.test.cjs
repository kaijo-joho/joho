...
(async()=>{
  ...
  await detail('series',measured.id);const measuredCell=dialog.locator('input[aria-label="1行2列"]');await measuredCell.click();await measuredCell.press('F2');await measuredCell.fill('21');await measuredCell.press('Tab');await submit();await Toolbar.selectWorkspace(page,scatter.id);await page.waitForFunction(()=>document.querySelector('#analysis-plot').data&&document.querySelector('#analysis-plot').data[0].y[0]===21);assert.equal((await doc()).series[0].dataTable.rows[0][1],21);
  ...
  await Toolbar.openSettings(page);await page.locator('[data-theme-value="dark"]').click();await page.keyboard.press('Escape');await page.setViewportSize({width:390,height:850});await page.waitForTimeout(150);await settle();if(await page.locator('#export-panel [data-close-side]').isVisible())await page.locator('#export-panel [data-close-side]').tap();await page.locator('#list-toggle').tap();await page.locator('#series-add-toggle').tap();await page.keyboard.press('Tab');await page.keyboard.press('Escape');await page.waitForTimeout(150);await settle();
