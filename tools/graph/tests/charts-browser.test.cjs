...
(async()=>{
  ...
  await page.locator('#view-menu summary').click();await page.locator('#theme').selectOption('dark');await page.keyboard.press('Escape');await page.setViewportSize({width:390,height:850});await page.waitForTimeout(150);await settle();if(await page.locator('#export-panel [data-close-side]').isVisible())await page.locator('#export-panel [data-close-side]').tap();await page.locator('#list-toggle').tap();await page.locator('#series-add-toggle').tap();await page.keyboard.press('Tab');await page.keyboard.press('Escape');await page.waitForTimeout(150);await settle();
