...
(async () => {
  ...
  assert.equal(await page.locator('.top #document-name').count(), 1);
  ...
  const order = ['.brand', '#file-menu', '#undo', '#redo', '#multiple-select', '#display-menu', '#document-name', '#view-menu', '#help-button'];
  ...
  assert.equal(await page.evaluate(() => localStorage.getItem('kaijo-graph:saved')), null, 'hover never saves');
