...
(async () => {
  ...
  assert.deepEqual(await doc(), saved); assert.equal((await doc()).version, 15);
  ...
  assert.equal((await doc()).version, 15); assert((await doc()).annotations.filter(a => a.kind === 'tangent').every(a => a.showEquation === false));
