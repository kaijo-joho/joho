...
(async () => {
  ...
  assert.deepEqual(await doc(), saved); assert.equal((await doc()).version, 10);
  ...
  const legacy = structuredClone(saved); legacy.version = 3;delete legacy.charts;delete legacy.comparison; delete legacy.presentation; delete legacy.output; legacy.name = '旧形式の読み込み確認';
  ...
  assert.equal((await doc()).version, 10); assert((await doc()).annotations.filter(a => a.kind === 'tangent').every(a => a.showEquation === false));
