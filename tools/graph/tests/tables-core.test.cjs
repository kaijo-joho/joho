...
}
{
  for (let version = 1; version <= 7; version++) {
    ...
    const clean = C.validateDocument(old); assert.equal(clean.version, 15, 'v' + version + ' migrates to v15');
  }
  for (let version = 1; version <= 7; version++) {
...
}
{
  ...
  store.save('auto', d); const loaded = store.load('auto').document; assert.equal(loaded.version, 15); assert.deepStrictEqual(loaded.series[0].dataTable, source.dataTable); assert.equal(loaded.annotations.find(a => a.id === 'point').anchor.regressionId, 'fit');
}
