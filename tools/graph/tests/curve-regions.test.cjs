...
{
 const d=base(),r=region(d);const clean=C.validateDocument(d);assert.equal(clean.version,6);assert.deepStrictEqual(clean.annotations[0].targets,r.targets);const extra=C.clone(d);extra.annotations[0].targets[0].extra='drop';assert.equal(C.validateDocument(extra).annotations[0].targets[0].extra,undefined);
...
{
 const old=C.clone(require('../templates.js').list().find(t=>t.id==='math-triangle-region').document);assert.equal(old.version,5);const migrated=C.validateDocument(old);assert.equal(migrated.version,6);assert.deepStrictEqual(migrated.annotations.find(a=>a.kind==='region').segmentIds,old.annotations.find(a=>a.kind==='region').segmentIds);
