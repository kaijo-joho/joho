...
{
 const d=base(),r=region(d);const clean=C.validateDocument(d);assert.equal(clean.version,9);assert.deepStrictEqual(clean.annotations[0].targets,r.targets);const extra=C.clone(d);extra.annotations[0].targets[0].extra='drop';assert.equal(C.validateDocument(extra).annotations[0].targets[0].extra,undefined);
