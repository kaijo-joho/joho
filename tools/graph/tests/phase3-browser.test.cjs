...
(async () => {
  ...
  const saved = JSON.parse(fs.readFileSync(await file.path(), 'utf8')); assert.equal(saved.version, 15); assert(saved.annotations.some(a => a.kind === 'segment'));
  ...
  assert.equal((await doc()).version,15);assert.equal((await doc()).axes.x.symbol,'x');assert.equal((await doc()).annotations[0].label.visible,true);
