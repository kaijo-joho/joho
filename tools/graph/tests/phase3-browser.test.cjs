...
(async () => {
  ...
  await page.locator('#add-function').click(); await fill('名前', '移動'); await fill('数式（例：s = a*t^2）', 's = 2*t'); await submit();
  ...
  await ann('AB').click(); await page.getByRole('button', { name: '文字・配置', exact: true }).click();
  ...
  await page.locator('#list-toggle').tap(); await ann('AB').tap(); await page.getByRole('button', { name: '文字・配置', exact: true }).tap(); await fill('文字サイズ（px）', 16); await submit();
  ...
  await submit();await page.locator('#add-function').click();await fill('数式（例：γ = α^2 + β^2）','γ = α^2 + β^2');await submit();
