...
(async () => {
  ...
  let d = await doc(); const region = d.annotations.find(a => a.kind === 'region'); assert(region); assert.equal(d.version, 7);
  ...
  const saved = JSON.parse(fs.readFileSync(await (await downloading).path(), 'utf8')); assert.equal(saved.version, 7);
