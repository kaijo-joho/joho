...
(async () => {
  ...
  const details = id => page.locator('[data-object-details="annotation:' + id + '"]');
  ...
  let d = await doc(); const region = d.annotations.find(a => a.kind === 'region'); assert(region); assert.equal(d.version, 5);
  ...
  await details('C').click(); await fill('y 座標', '0'); await submit();
  ...
  const valid = await doc(); await details(region.id).click(); await dialog.locator('[data-region-segment="AB"]').uncheck(); await page.locator('#dialog-submit').click();
  assert(await page.locator('#dialog-error').isVisible()); assert.deepEqual(await doc(), valid); await page.keyboard.press('Escape');
  await item('A').click(); await bar.getByRole('button', { name: '削除', exact: true }).click(); await settle();
  ...
  const saved = JSON.parse(fs.readFileSync(await (await downloading).path(), 'utf8')); assert.equal(saved.version, 5);
  ...
  await page.setViewportSize({ width: 390, height: 850 }); await page.locator('#list-toggle').tap(); await page.locator('#annotation-add-toggle').tap(); await page.locator('#add-region').tap();
  ...
  await page.keyboard.press('Escape'); assert(await page.locator('#annotation-add-toggle').evaluate(el => el === document.activeElement));
