...
}
{
  ...
  const clean = Core.validateDocument(document); assert.equal(clean.version, 7); Core.removeAnnotation(document, 'a'); assert.equal(document.annotations.length, 3, '点→線分→領域を再帰削除する'); assert(!document.annotations.some(annotation => annotation.id === 'r'));
