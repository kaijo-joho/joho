...
(async () => {
  ...
  await bar.getByLabel('自由な色（RGB）',{exact:true}).fill('#0891b2'); await settle(); assert(!await dialog.isVisible()); assert.equal((await doc()).series[0].style.color, '#0891b2');
  await width.fill('4'); await width.press('Tab'); await settle();
  assert.equal((await doc()).series[0].style.width, 4);
  assert(await bar.getByLabel('線の太さ（スライダー）', { exact: true }).evaluate(el => el === document.activeElement), 'Tab advances to the next direct control');
  await bar.getByRole('button', { name:'点線',exact: true }).click(); await settle(); assert.equal((await doc()).series[0].style.dash, 'dot');
  ...
  await bar.getByLabel('自由な色（RGB）',{exact:true}).fill('#0c2238'); await settle();
