...
(async () => {
  ...
  const addRegion = async () => { if (await page.locator('#annotation-add-panel').isHidden()) await page.locator('#annotation-add-toggle').click(); await page.locator('#add-region').click(); await dialog.waitFor({ state: 'visible' }); };
  ...
  let d = await doc(), curveRegion = d.annotations.find(a => a.kind === 'curveRegion'); assert(curveRegion); assert.equal(d.version, 10); assert.equal(curveRegion.targets[0].type, 'series'); assert.equal(curveRegion.targets[1].type, 'series');
  ...
  const detail=page.locator('[data-object-details="annotation:'+curveRegion.id+'"]'),beforeInterval=await doc();
  await detail.click();await dialog.getByLabel('x の終点',{exact:true}).fill('2');await page.locator('#dialog-submit').click();assert(await page.locator('#dialog-error').isVisible());assert.deepEqual(await doc(),beforeInterval);
  ...
  await page.locator('[data-object-id="f"]').click();await bar.getByRole('button',{name:'削除',exact:true}).click();await settle();assert.equal((await doc()).annotations.length,0);
  await page.locator('#undo').click();await settle();assert.deepEqual(await doc(),beforeInterval);
  ...
  await page.locator('#file-menu summary').click(); const download = page.waitForEvent('download'); await page.locator('#save-local').click(); const saved = JSON.parse(fs.readFileSync(await (await download).path(), 'utf8')); assert.equal(saved.version, 10); await importDoc(C.createDocument()); await importDoc(saved); assert.equal((await doc()).version, 10);
  await page.locator('#file-menu summary').click(); await page.locator('#save-browser').click(); assert.equal((await page.evaluate(() => JSON.parse(localStorage.getItem('kaijo-graph:saved')).document.version)), 10);
  assert.deepEqual(await doc(),saved);assert.deepEqual(await page.evaluate(()=>JSON.parse(localStorage.getItem('kaijo-graph:saved')).document),saved);
  ...
  await page.locator('#list-toggle').tap();await page.locator('#annotation-add-toggle').tap();await page.locator('#add-region').tap();await dialog.waitFor({ state: 'visible' });
  ...
  await page.keyboard.press('Escape');assert(await page.locator('#annotation-add-toggle').evaluate(el => el === document.activeElement));assert.deepEqual(errors, []);
