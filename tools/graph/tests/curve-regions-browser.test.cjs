...
(async () => {
  ...
  let d = await doc(), curveRegion = d.annotations.find(a => a.kind === 'curveRegion'); assert(curveRegion); assert.equal(d.version, 13); assert.equal(curveRegion.targets[0].type, 'series'); assert.equal(curveRegion.targets[1].type, 'series');
  ...
  await page.locator('#file-menu summary').click(); const download = page.waitForEvent('download'); await page.locator('#save-local').click(); const saved = JSON.parse(fs.readFileSync(await (await download).path(), 'utf8')); assert.equal(saved.version, 13); await importDoc(C.createDocument()); await importDoc(saved); assert.equal((await doc()).version, 13);
  await page.locator('#file-menu summary').click(); await page.locator('#save-browser').click(); assert.equal((await page.evaluate(() => JSON.parse(localStorage.getItem('kaijo-graph:saved')).document.version)), 13);
