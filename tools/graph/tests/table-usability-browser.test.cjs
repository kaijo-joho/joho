...
(async()=>{
  ...
  assert.equal(await editor.locator('.graph-table-editor__tools').count(),0,'旧ツール枠を残さない');
  assert.equal(await editor.locator('.graph-table-editor__mapping-details').getAttribute('open'),'','広幅では列割当を常時表示する');
  assert.equal(await editor.locator('.graph-table-editor__column-menu summary svg[aria-hidden="true"]').count(),1,'列追加は表右端のアイコンから開く');
  assert.equal(await editor.locator('.graph-table-editor__table-actions').count(),1,'行追加は表の下に置く');
  await editor.getByLabel('横軸の列',{exact:true}).selectOption('1');
  await editor.locator('.graph-table-editor__column-menu summary').click();await editor.getByRole('button',{name:'計算列を追加',exact:true}).click();await editor.getByLabel('計算列の名前',{exact:true}).fill('下書き');await editor.getByLabel('計算式',{exact:true}).fill('[@x] * 2');
  ...
  await editor.locator('.graph-table-editor__column-menu summary').click();assert.equal(await editor.getByRole('button',{name:'最後の列を削除',exact:true}).isVisible(),true,'既存の削除操作を列メニューから利用できる');
