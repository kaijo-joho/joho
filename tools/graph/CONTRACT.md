...
現行0.23.0では、2Dは軸の直接クリック、3Dは右パネルの横・縦・高さ選択で対象軸を選び、軸線・目盛・グリッドを軸ごとに書式設定する。分析グラフ、比較、散布図行列にも反映し、行列は横・縦方向別に設定する。既存の座標・目盛の詳細設定は併存する。3Dのグリッド線種は実線のみとする。保存形式はversion 15で、`axes.*.style` の未指定項目は従来の全体設定・テーマを使う。0.22.0の複数文書・保存先・終了確認と右側タブは [複数文書と右側タブ](docs/document-tabs.md) を参照する。0.21.0の上部共通仕様への適用は [上部ツールバー](docs/toolbar-layout.md) を参照する。
...
- axis.style: 任意。`color` と `gridColor` は`#rrggbb`または`null`（自動色）、`width` と `gridWidth` は0.5〜10、`grid`・`tickMarks`・`tickLabels`はboolean、`gridDash`は`solid`/`dash`/`dot`。必要な項目だけ保存する。
...
- `GraphAxisStyle.validate(style)` は指定済みの既知項目だけを検証して返す。`GraphAxisStyle.apply(layoutAxis,style,{foreground,gridColor})` は明示した項目だけをPlotly軸設定へ適用する。
...
- `GraphPlot.resetView(element,doc)` / `resize(element)` / `pickObject(element,event)`。`pickObject`は2Dの描画済み線・点の選択メタデータを返す。
- `GraphTemplates.list()`：`[{id,name,category,description,document}]` の深いコピー。文書に図の元データと出典を含める。

## 操作の構成

上部1行、狭い幅はメニュー。追加は左の1つの「＋追加」に集約し、選択対象は右ペインの「書式」タブで編集する。軸は2Dの直接クリックまたは右パネルの横・縦・高さ選択で対象を決め、個別書式と既存の詳細設定を同じパネルで扱う。右のタブでテンプレートと書き出しを扱い、共通非モーダルヘルプを使う。詳細な配置・並べ替え・ズーム操作は [右インスペクターの配置契約](docs/inspector-layout.md) に従う。
