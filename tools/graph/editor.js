/* global GraphCore, GraphExpression, GraphCurves, GraphAnnotations, GraphPlot, GraphTemplates, IlapoLocalAutosave, JohoToolHelp */
(function () {
  'use strict';
  ...
  const annotationNames = {point:'点',guide:'補助線',tangent:'接線',intersection:'交点'};
  ...
  function curveLabel(s) { return s.kind==='parametric' ? 'x = '+s.components.x+' / y = '+s.components.y : s.kind==='polar' ? 'r = '+s.expression : s.expression; }
  ...
  function updateToolbar() {
    ...
    if(s) {
      ...
      bar.append(node('span',a.name||annotationNames[a.kind],{class:'selection-name'}),button('位置・設定',()=>editAnnotation(a.id)),button('色・線',()=>editStyle(a.id,'annotation')));
  ...
  function removeAnnotation(id) { changed(d=>{d.annotations=d.annotations.filter(a=>a.id!==id);});select(null); }
  ...
  function editSeries(id, supplied) {
    ...
    openDialog((existing?'編集：':'追加：')+kindNames[s.kind],parent=>{
      ...
      if(isData){table=area(parent,is3?'数表（x, y, z の3列）':'数表（x, y の2列）',C.tableCSV(s.rows));table.placeholder=is3?'x,y,z\n0,0,0\n1,2,3':'x,y\n0,0\n1,1\n2,4';parent.append(node('p','CSVまたはタブ区切り。欠測のセルは空欄。点は入力した順に結びます。',{class:'small muted'}));
        ...
        if(isParam){componentX=field(parent,'x(t) の式',s.components.x,'text',formulaAttrs);componentY=field(parent,'y(t) の式',s.components.y,'text',formulaAttrs);}
        else expression=field(parent,is3?'数式（例：z = x^2 + y^2）':isImplicit?'方程式（例：x^2 + y^2 = 9）':isPolar?'r の式（例：2*cos(3*theta)）':'数式（例：y = a*x^2）',s.expression,'text',formulaAttrs);
        ...
        else{xmin=field(g,'x の最小値',s.domain.x[0],'number',{step:'any'});xmax=field(g,'x の最大値',s.domain.x[1],'number',{step:'any'});if(is3||isImplicit){ymin=field(g,'y の最小値',s.domain.y[0],'number',{step:'any'});ymax=field(g,'y の最大値',s.domain.y[1],'number',{step:'any'});}}
      ...
      else{if(isParam)s.components={x:componentX.value.trim(),y:componentY.value.trim()};else s.expression=expression.value.trim();if(isParam||isPolar)s.interval=[requiredNumber(intervalMin),requiredNumber(intervalMax)];else{s.domain.x=[requiredNumber(xmin),requiredNumber(xmax)];if(is3||isImplicit)s.domain.y=[requiredNumber(ymin),requiredNumber(ymax)];}}
  ...
  function addAnnotation(kind,seriesId) {
    if(!kind){openDialog('点・補助線を追加',parent=>{for(const [value,label]of Object.entries(annotationNames))parent.append(button(label+'を追加',()=>addAnnotation(value),{class:'full-button'}));parent.append(node('p','点・補助線は2Dで表示します。接線と交点は y = f(x) が対象です。',{class:'small muted'}));},null);return;}
  ...
  function editAnnotation(id,supplied) {
    ...
    let name,pointType,curve,at,x,y,projections,axis,value,first,second,min,max;
    const curves=current().series.filter(s=>curveKinds.includes(s.kind)), functions=current().series.filter(s=>s.kind==='function');
    const options=items=>items.map(s=>[s.id,s.name||curveLabel(s)]),exprAttrs={maxlength:1000,spellcheck:'false',autocapitalize:'none',autocomplete:'off'};
    openDialog((existing?'編集：':'追加：')+annotationNames[a.kind],parent=>{
      ...
      if(a.kind==='point'){
        ...
        x=field(freeBox,'x 座標',a.anchor.x||'0','text',exprAttrs);y=field(freeBox,'y 座標',a.anchor.y||'0','text',exprAttrs);
        curve=choice(curveBox,'対象の曲線',a.anchor.seriesId||curves[0]?.id,options(curves));at=field(curveBox,'位置（x / t / theta の値）',a.anchor.at||'0','text',exprAttrs);
        const hint=node('p','',{class:'small muted'});curveBox.append(hint);
        const refresh=()=>{freeBox.hidden=pointType.value!=='free';curveBox.hidden=pointType.value!=='curve';const s=curves.find(s=>s.id===curve.value);hint.textContent=s?(s.kind==='function'?'x':s.kind==='parametric'?'t':'theta')+' の値を指定します。対象の定義範囲内で入力してください。':'';};pointType.addEventListener('change',refresh);curve.addEventListener('change',refresh);refresh();
        projections=check(parent,'軸への読取線を表示',a.projections);
      }else if(a.kind==='guide'){axis=choice(parent,'方向',a.axis,[['x','縦の線（x = 一定）'],['y','横の線（y = 一定）']]);value=field(parent,'座標の値',a.value,'text',exprAttrs);}
      else if(a.kind==='tangent'){curve=choice(parent,'対象の数式',a.seriesId,options(functions));at=field(parent,'接点の x 座標',a.at,'text',exprAttrs);parent.append(node('p','接点と接線を数値計算で描きます。角や未定義の点では接線を表示しません。',{class:'small muted'}));}
      else{first=choice(parent,'1つ目の数式',a.seriesIds[0],options(functions));second=choice(parent,'2つ目の数式',a.seriesIds[1],options(functions));const g=grid(parent);min=field(g,'探索する x の最小値',a.interval[0],'number',{step:'any'});max=field(g,'探索する x の最大値',a.interval[1],'number',{step:'any'});parent.append(node('p','指定区間と両方の定義範囲が重なる部分を数値探索します。非常に近い交点や細かい振動は捉えきれないことがあります。',{class:'small muted'}));}
      if(a.kind!=='intersection')parent.append(node('p','座標や位置には 1/2、sqrt(2)、登録した係数などの数式も使えます。',{class:'small muted'}));
    },()=>{
      a.name=name.value.trim();
      if(a.kind==='point'){a.anchor=pointType.value==='free'?{type:'free',x:x.value.trim(),y:y.value.trim()}:{type:'curve',seriesId:curve.value,at:at.value.trim()};a.projections=projections.checked;}
      else if(a.kind==='guide'){a.axis=axis.value;a.value=value.value.trim();}
      else if(a.kind==='tangent'){a.seriesId=curve.value;a.at=at.value.trim();}
      else{a.seriesIds=[first.value,second.value];a.interval=[requiredNumber(min),requiredNumber(max)];}
      changed(d=>{if(existing)d.annotations[d.annotations.findIndex(x=>x.id===id)]=a;else d.annotations.push(a);});select({type:'annotation',id:a.id});
    },'描画');
  }
  ...
  function editAxes() {
    ...
    openDialog('座標・グラフの設定',parent=>{
      ...
      parent.append(node('p','軸名や単位は表示用です。角度を変更しても入力済みの範囲は自動換算しません。極座標で1周する範囲はラジアンで約6.283185307、度で360です。',{class:'small muted'}));
  ...
  function fitView() {
    ...
    if(doc.mode==='2d')for(const a of doc.annotations.filter(a=>a.visible)){const result=GraphAnnotations.evaluate(a,doc);for(const p of result.points){add('x',p[0]);add('y',p[1]);}if(a.kind==='guide'&&result.segments.length)add(a.axis,result.segments[0][0][a.axis==='x'?0:1]);}
